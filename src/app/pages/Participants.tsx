    ...
    return (
        <div className="space-y-6 text-foreground">
            ...
            <Dialog open={deleteModal.open} onOpenChange={(open) => { if (!open && !isDeleting) setDeleteModal({ ...deleteModal, open: false }) }}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{t('personnel.alerts.removeParticipantConfirmTitle', 'Remove Participant?')}</DialogTitle>
                        <DialogDescription>
                            {t('personnel.alerts.removeParticipantConfirm', { name: deleteModal.name, defaultValue: `Are you sure you want to remove participant "${deleteModal.name}"? All associated training data for this user will remain in history but they will lose access.` })}
                        </DialogDescription>
                    </DialogHeader>
