                        ...
                        >
                            ...
                            <div className="flex items-center justify-end w-20 shrink-0 gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity absolute right-4 top-4 sm:static sm:right-auto sm:top-auto">
                                <Button
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        requestDelete(officer.id, officer.name);
                                    }}
    ...
    return (
        <div className="space-y-10 text-foreground pb-12">
            ...
            <Dialog open={deleteModal.open} onOpenChange={(open) => { if (!open && !isDeleting) setDeleteModal({ ...deleteModal, open: false }) }}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{t('personnel.alerts.removeOfficerConfirmTitle', 'Remove Officer?')}</DialogTitle>
                        <DialogDescription>
                            {t('personnel.alerts.removeOfficerConfirm', { name: deleteModal.name, defaultValue: `Are you sure you want to remove this officer "${deleteModal.name}"?` })}
                        </DialogDescription>
                    </DialogHeader>
