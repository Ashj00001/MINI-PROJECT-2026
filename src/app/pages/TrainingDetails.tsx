...
import { MyFeedbackSubmission, TrainingFeedback, TrainingFeedbackSummary, Institution } from '../../types';
import {
...
const TrainingDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [trainingRes, instData] = await Promise.all([
    ...
    const getInstitutionName = (institutionId: any) => {
        ...
        if (typeof institutionId === 'object' && institutionId.name) return institutionId.name;
        const instId = typeof institutionId === 'object' ? institutionId.id || institutionId._id : institutionId;
        const inst = institutions.find(i => i.id === instId || (i as any)._id === instId);
                        ...
                        <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/10">
                            <div>
                                ...
                                <div className="flex flex-col gap-1.5">
                                    {Array.isArray(training.targetAudience) && training.targetAudience.length > 0 ? (
                                        training.targetAudience.map((audience: string, index: number) => (
                                            <Badge key={index} variant="outline" className="w-fit font-medium border-primary/20 text-foreground bg-primary/5">
                                                {audience}
                                            </Badge>
                                        ))
                                    ) : (
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-1.5">
                                    {Array.isArray(training.requiredInstitutions) && training.requiredInstitutions.length > 0 ? (
                                        training.requiredInstitutions.map((instId: any, index: number) => (
                                            <Badge key={index} variant="outline" className="w-fit text-foreground bg-secondary/10 font-medium whitespace-normal text-left">
                                                {getInstitutionName(instId)}
                                            </Badge>
                                        ))
                                    ) : (
                                        <p className="font-medium text-foreground">No specific institutions required</p>
                                    )}
                                </div>
                            </div>
                        </div>
