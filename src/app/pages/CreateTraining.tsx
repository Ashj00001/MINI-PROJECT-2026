...
const getInitialPrograms = () => {
  try {
    const custom = JSON.parse(localStorage.getItem('customPrograms') || '[]');
    return [...DEFAULT_PROGRAMS.filter(p => p !== 'Other'), ...custom, 'Other'];
  } catch {
...
const CreateTraining: React.FC = () => {
  ...
  const [programsList, setProgramsList] = useState<string[]>(getInitialPrograms());
      ...
      try {
        ...
        if (isEditMode && id) {
          try {
            ...
            const isCustomProgram = !programsList.includes(training.program);
            ...
            setFormData({
              ...
              targetAudience: (() => {
                ...
                const standards = TARGET_AUDIENCES;
        ...
        } else if (location.state?.prefilledSchedule) {
          const schedule = location.state.prefilledSchedule;
          setFormData(prev => ({
            ...prev,
            date: schedule.date ? new Date(schedule.date).toISOString().split('T')[0] : prev.date,
            startTime: schedule.startTime || prev.startTime,
            endTime: schedule.endTime || prev.endTime,
          }));
          window.history.replaceState({}, document.title);
  ...
  const handleAddCustomProgram = (programName: string) => {
    const p = programName.trim();
    if (!p) return;
    
    let customProgs: string[] = [];
    try {
      customProgs = JSON.parse(localStorage.getItem('customPrograms') || '[]');
    } catch {}

    if (!customProgs.includes(p)) {
      customProgs.push(p);
      localStorage.setItem('customPrograms', JSON.stringify(customProgs));
      
      setProgramsList(prev => {
        const newList = prev.filter(x => x !== 'Other');
        newList.push(p, 'Other');
        return newList;
      });
    }

    handleChange('program', p);
    handleChange('customProgram', '');
  };
              ...
              <div>
                ...
                {formData.program === 'Other' && (
                  <div className="mt-3 flex gap-2">
                    <div className="flex-1">
                      <Input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            if (formData.customProgram.trim()) {
                              handleAddCustomProgram(formData.customProgram);
                            }
                          }
                        }}
                      />
                    ...
                    <Button 
                      type="button" 
                      onClick={() => handleAddCustomProgram(formData.customProgram)}
                      disabled={!formData.customProgram.trim()}
                    >
                      {t('createTraining.buttons.addProgram', 'Add')}
                    </Button>
                  </div>
                )}
                ...
                <Select
                  ...
                  onValueChange={(val) => {
                    if (val === 'morning') {
                      ...
                      setFormData(prev => {
                        const isPreset = 
                          (prev.startTime === '10:00' && prev.endTime === '13:30') ||
                          (prev.startTime === '14:00' && prev.endTime === '17:00') ||
                          (prev.startTime === '10:00' && prev.endTime === '17:00');
                        return { 
                          ...prev, 
                          startTime: isPreset ? '09:00' : (prev.startTime || '09:00'), 
                          endTime: isPreset ? '17:00' : (prev.endTime || '17:00') 
                        };
                      });
                      setErrors(prev => ({ ...prev, startTime: '', endTime: '' }));
                    }
                ...
                <div className="mt-3 grid grid-cols-1 gap-4 xl:grid-cols-2">
                  {hallCards.map((hall, index) => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => {
                        handleChange('hallId', hall.id);
                        if (formData.capacity && parseInt(formData.capacity, 10) > hall.capacity) {
                          handleChange('capacity', hall.capacity.toString());
                          toast.info(`Capacity auto-adjusted to match hall maximum (${hall.capacity} seats)`);
                        }
                      }}
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedHall && val) {
                      ...
                      if (num > selectedHall.capacity) {
                        val = selectedHall.capacity.toString();
                        toast.error(`Capacity cannot exceed hall capacity (${selectedHall.capacity})`);
                      }
                    }
                  ...
                  min="1"
                  max={selectedHall ? selectedHall.capacity : undefined}
                  className="mt-2"
                />
                {selectedHall && (
