...
const Trainings: React.FC = () => {
  ...
  const [abortModalState, setAbortModalState] = useState<{ open: boolean, training: Training | null, step: 'confirm' | 'reschedule' }>({ open: false, training: null, step: 'confirm' });
  const [isAborting, setIsAborting] = useState(false);

  const handleAbortClick = (training: Training) => {
    setAbortModalState({ open: true, training, step: 'confirm' });
  };

  const executeAbort = async () => {
    const training = abortModalState.training;
    if (!training) return;
    setIsAborting(true);
    try {
      await api.patch(`/trainings/${training.id}/status`, { status: 'cancelled' });
      setTrainings(prev => prev.map(t => t.id === training.id ? { ...t, status: 'cancelled' } : t));
      toast.success(t('trainings.abortedSuccess', 'Aborted'));
      setAbortModalState({ open: true, training, step: 'reschedule' });
    } catch (e) {
      toast.error('Failed to abort');
      setAbortModalState({ open: false, training: null, step: 'confirm' });
    } finally {
      setIsAborting(false);
    }
  };
  ...
  return (
    <div className="pb-12 space-y-6 text-foreground">
                    ...
                    {((user?.role === 'program_officer' && training.createdById === user.id) || user?.role === 'master_admin') && (training.status === 'scheduled' || training.status === 'ongoing') && (
                      <>
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => handleAbortClick(training)}
      ...
      {abortModalState.training && (
        <Dialog open={abortModalState.open} onOpenChange={(open) => { if (!open && !isAborting) setAbortModalState({ ...abortModalState, open: false }) }}>
          <DialogContent>
            {abortModalState.step === 'confirm' ? (
              <>
                <DialogHeader>
                  <DialogTitle>{t('trainings.confirmAbort', 'Abort mission?')}</DialogTitle>
                  <DialogDescription>
                    Are you sure you want to abort this scheduled training? This action cannot be undone.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter className="mt-4">
                  <Button variant="outline" onClick={() => setAbortModalState({ ...abortModalState, open: false })} disabled={isAborting}>
                    Cancel
                  </Button>
                  <Button variant="destructive" onClick={executeAbort} disabled={isAborting}>
                    {isAborting ? 'Aborting...' : 'Abort Meeting'}
                  </Button>
                </DialogFooter>
              </>
            ) : (
              <>
                <DialogHeader>
                  <DialogTitle>Reschedule Options</DialogTitle>
                  <DialogDescription>
                    Would you like to schedule a new replacement training on the same date and time slot?
                  </DialogDescription>
                </DialogHeader>
                <div className="bg-secondary/30 p-3 rounded-md text-sm my-2">
                  <p><span className="font-semibold">Original Meeting:</span> {abortModalState.training.title}</p>
                  <p><span className="font-semibold">Slot:</span> {safeFormatDate(abortModalState.training.date)} | {abortModalState.training.startTime} - {abortModalState.training.endTime}</p>
                </div>
                <DialogFooter className="mt-4 flex sm:justify-between">
                  <Button variant="outline" onClick={() => setAbortModalState({ ...abortModalState, open: false })}>
                    No, I'm done
                  </Button>
                  <Button onClick={handleRescheduleYes}>
                    Yes, Reschedule Now
                  </Button>
                </DialogFooter>
              </>
