...
const TrainingParticipants: React.FC = () => {
    ...
    const [deleteModal, setDeleteModal] = useState<{ open: boolean, type: 'single' | 'bulk', id?: string, name?: string }>({ open: false, type: 'single' });
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Only show approved and attended participants who perfectly match the training criteria
            const activeParticipants = nominationsData.filter(nom => {
                if (nom.status !== 'approved' && nom.status !== 'attended') return false;
                
                const u = nom.participant;
                if (u && trainingData) {
                    const getSafeInstId = (instId: any) => typeof instId === 'object' && instId ? instId.id || instId._id : instId;
                    
                    // Check explicit institutions
                    if (trainingData.requiredInstitutions && trainingData.requiredInstitutions.length > 0) {
                        const uInstId = getSafeInstId(u.institutionId);
                        const reqInstIds = trainingData.requiredInstitutions.map((id: any) => getSafeInstId(id));
                        if (!uInstId || !reqInstIds.includes(uInstId)) return false;
                    }

                    // Check explicit target audiences
                    if (trainingData.targetAudience && trainingData.targetAudience.length > 0) {
                        const uDesig = u.designation?.toLowerCase().trim();
                        const targetAuds = trainingData.targetAudience.map((ta: string) => ta?.toLowerCase().trim());
                        if (!uDesig || !targetAuds.includes(uDesig)) return false;
                    }
                }
    ...
    const requestRemoveParticipant = (nominationId: string, participantName: string) => {
        setDeleteModal({ open: true, type: 'single', id: nominationId, name: participantName });
    };
    ...
    const confirmAction = async () => {
        if (deleteModal.type === 'single' && deleteModal.id) {
            setIsDeleting(true);
            try {
                await nominationsApi.updateStatus(deleteModal.id, 'rejected', 'Removed from training by Administrator/Program Officer');
                toast.success(t('participantsManage.removedSuccess', { name: deleteModal.name }));
                fetchData();
            } catch (error) {
    ...
    return (
        <div className="space-y-6">
                                        ...
                                        <TableRow key={nom.id} className={selectedParticipantIds.includes(nom.id) ? "bg-muted/50" : ""}>
                                            ...
                                            <TableCell className="text-right">
                                                <Button
                                                    variant="destructive"
                                                    size="sm"
                                                    onClick={() => requestRemoveParticipant(nom.id, nom.participant?.name || t('participantsManage.unknown'))}
            ...
            <Dialog open={deleteModal.open} onOpenChange={(open) => { if (!open && !isDeleting) setDeleteModal({ ...deleteModal, open: false }) }}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {deleteModal.type === 'single' 
                                ? t('participantsManage.confirmRemoveTitle', 'Remove Participant?') 
                                : t('participantsManage.bulkRemoveTitle', 'Bulk Remove Participants?')}
                        </DialogTitle>
                        <DialogDescription>
                            {deleteModal.type === 'single'
                                ? t('participantsManage.confirmRemove', { name: deleteModal.name })
                                : t('participantsManage.bulkRemoveConfirm', { count: selectedParticipantIds.length })}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="mt-4">
                        <Button variant="outline" onClick={() => setDeleteModal({ ...deleteModal, open: false })} disabled={isDeleting}>
                            Cancel
                        </Button>
                        <Button variant="destructive" onClick={confirmAction} disabled={isDeleting}>
