...
const ScanQR: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const qrScanner = new Html5QrcodeScanner(
      'qr-reader',
      {
        fps: 30, // Increased frame rate to capture unblurred frames faster
        qrbox: (viewfinderWidth: number, viewfinderHeight: number) => {
          // Dynamic box covering 80% of the screen stream for easier aiming
          const minEdgeSize = Math.min(viewfinderWidth, viewfinderHeight);
          const qrboxSize = Math.floor(minEdgeSize * 0.85);
          return { width: qrboxSize, height: qrboxSize };
        },
        formatsToSupport: [Html5QrcodeSupportedFormats.QR_CODE], // Only decode QR codes, massive structural speedup
        showTorchButtonIfSupported: true,
        rememberLastUsedCamera: true,
        videoConstraints: {
          facingMode: "environment",
          width: { ideal: 1920, min: 1280 }, // Request high density video
