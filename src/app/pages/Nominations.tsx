...
const Nominations: React.FC = () => {
  ...
  try {
    const filteredNominations = nominations.filter(nom => {
      // 1. Enforce strict requirements validation
      const nTraining = trainings.find(t => t.id === nom.trainingId);
      const u = users.find(usr => usr.id === nom.participantId);
      
      if (nTraining && u) {
          const getSafeInstId = (instId: any) => typeof instId === 'object' && instId ? instId.id || instId._id : instId;
          
          if (nTraining.requiredInstitutions && nTraining.requiredInstitutions.length > 0) {
              const uInstId = getSafeInstId(u.institutionId);
              const reqInstArray = Array.isArray(nTraining.requiredInstitutions) ? nTraining.requiredInstitutions : [nTraining.requiredInstitutions];
              const reqInstIds = reqInstArray.map((id: any) => getSafeInstId(id));
              if (!uInstId || !reqInstIds.includes(uInstId)) return false;
          }

          if (nTraining.targetAudience && nTraining.targetAudience.length > 0) {
              const uDesig = u.designation?.toLowerCase().trim();
              const targetAudArray = Array.isArray(nTraining.targetAudience) ? nTraining.targetAudience : [nTraining.targetAudience];
              const targetAuds = targetAudArray.map((ta: string) => ta?.toLowerCase().trim());
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-hidden">
            <div className="bg-background border border-border rounded-xl w-full max-w-lg p-6 shadow-xl max-h-[90vh] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
                ...
                <div>
                  <Label className="text-sm font-medium text-foreground mb-2 block">{t('nominationsProps.selectedParticipants', { defaultValue: 'Selected participants' })} ({selectedParticipantIds.length})</Label>
                  <div className="space-y-2">
                    {users.filter(u => {
                      ...
                      if ((user.role === 'medical_officer' || user.role === 'institutional_admin') && uInstId !== getSafeInstId(user.institutionId)) {
                        return false;
                      }
                      
                      // Match user institution and designation
                      if (selectedTrainingId) {
                        const training = trainings.find(t => t.id === selectedTrainingId);
                        
                        // Check explicit institutions
                        if (training && training.requiredInstitutions && training.requiredInstitutions.length > 0) {
                          const reqInstArray = Array.isArray(training.requiredInstitutions) ? training.requiredInstitutions : [training.requiredInstitutions];
                          const reqInstIds = reqInstArray.map((id: any) => getSafeInstId(id));
                          if (!uInstId || !reqInstIds.includes(uInstId)) {
                            return false;
                          }
                        }

                        // Check explicit target audiences
                        if (training && training.targetAudience && training.targetAudience.length > 0) {
                          const uDesig = u.designation?.toLowerCase().trim();
                          const targetAudArray = Array.isArray(training.targetAudience) ? training.targetAudience : [training.targetAudience];
                          const targetAuds = targetAudArray.map((ta: string) => ta?.toLowerCase().trim());
                          if (!uDesig || !targetAuds.includes(uDesig)) {
                            return false;
                          }
                        }
                      }
